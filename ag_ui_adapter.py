...
async def run_agent(request: Request):
    ...
    # 🎯 FALLBACK LOGIC: Use last thread if current not found
    ...
    async def event_stream() -> AsyncGenerator[str, None]:
                ...
                if isinstance(tool_result, dict) and tool_result.get("ok"):
                    ...
                    if "formatted_summary" in result_data:
                        ...
                        if isinstance(delay_data, dict) and 'flightLegState' in delay_data:
                            ...
                            formatted_summary += f"Route: {flight_info.get('startStation', '')} → {flight_info.get('endStation', '')}\n"
        ...
        for step_index, step in enumerate(plan):
            ...
            try:
                ...
                if (tool_name == "get_delay_summary" and 
                    ...
                    # Send total token usage before finishing (only planning phase happened)
                    if planning_usage:
                        yield sse_event({
                            "type": "TOKEN_USAGE",
                            "phase": "total",
        ...
        try:
            ...
            if summarization_usage:
                ...
                # If no summarization usage but we have planning, send planning as total
                yield sse_event({
                    "type": "TOKEN_USAGE",
                    "phase": "total",
                    "usage": planning_usage
            ...
            # If summarization fails, still send planning usage as total
            if planning_usage:
                yield sse_event({
                    "type": "TOKEN_USAGE",
                    "phase": "total",
